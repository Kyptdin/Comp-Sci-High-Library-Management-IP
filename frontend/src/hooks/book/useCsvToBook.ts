...
export const useCsvToBook = (bookDataContainer: dataInterface[] | undefined) => {
    const { mutateAsync: createBook } = useCreateBook();
    const [booksValidated, setBooksValidated] = 
        useState<ValidateBookEntry[]>([]);

    const addValidatedBook = async (validatedBook: ValidateBookEntry) => {
        setBooksValidated((oldValidatedBooks) => 
            [...oldValidatedBooks, validatedBook]);
